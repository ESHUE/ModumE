...
public interface UserBoardRepository extends JpaRepository<Userboard_JPA, Integer>{
	@Query(value = "SELECT A.USERBOARD_SEQ, A.title, A.content, A.hits, DATE_FORMAT(A.r_date, '%m-%d') AS r_date, B.username, B.profileImg FROM Userboard_JPA A JOIN User_JPA B ON A.MAIN_SEQ = B.MAIN_SEQ")
