...
public interface UserBoardRepository extends JpaRepository<Userboard_JPA, Integer>{
	@Query(value = "SELECT A.boardseq, A.title, A.content, A.hits, DATE_FORMAT(A.rdate, '%m-%d'), A.convertcontent, B.username, B.profileImg FROM Userboard_JPA A JOIN User_JPA B ON A.userseq = B.userseq")
