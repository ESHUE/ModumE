...
public interface UserBoardRepository extends JpaRepository<Userboard_JPA, Integer>{
	@Query(value = "SELECT A.boardSeq, A.title, A.content, A.hits, DATE_FORMAT(A.rDate, '%m-%d') AS boardRDate, B.username, B.profileImg FROM Userboard_JPA A JOIN User_JPA B ON A.userSeq = B.userSeq")
